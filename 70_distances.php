<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN"> 

<html>

<head>

<title>CREEDO'S GUNBOUND CRAP</title>

<style type="text/css">

body { margin: 0; padding: 10px; background: #eef; color: #006; font: normal 1em/1.4em Arial, Tahoma, Helvetic, sans-serif; }

a { color: #0cc; font-weight: bold; text-decoration: none; }
a:hover { color: #f00; background: #ffe; }

h1 { color: #00f; font: bold 2.0em/2.2em; }

blockquote { width: 90%; text-align: justify; margin: 2ex; 1em; }

img { margin: 10px 0px; }

</style>

</head>

<body>Here are the powers for using armor/turtle<br>
with a fixed angle 70. Use my own windchart<br>
and/or the banpao windchart with these.<br>
Link to mine:<br>
<a href="windchart.jpg">Windchart</a><br>
Be sure to see the notes from the main page (under my stuff top left).<br>
<br>
Angle 70<br>
----------<br>
<br>
1.65 1/4 SD<br>
2.05 1/2 SD<br>
2.55 3/4 SD<br>
3.05 1.0 SD<br>
3.70 1.5 SD<br><br>
</body>
</html>

